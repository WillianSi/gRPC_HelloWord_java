...
public class HelloWorldService extends HelloWorldGrpc.HelloWorldImplBase {
    ...
    public void helloMessage(HelloWorldClass.HelloRequest request, StreamObserver<HelloWorldClass.HelloResponse> responseObserver) {
        HelloWorldClass.HelloResponse response = HelloWorldClass.HelloResponse.newBuilder().setMessage(MessageFormat
                .format("Hello {0}!", request.getName())).build();

        responseObserver.onNext(response);
        responseObserver.onCompleted();
    }
    @Override
    public void helloStream(HelloWorldClass.HelloRequest request, StreamObserver<HelloWorldClass
            .HelloResponse> responseObserver) {
        IntStream.range(1, 6).forEach(i -> {
            HelloWorldClass.HelloResponse response = HelloWorldClass.HelloResponse.newBuilder()
                    .setMessage(MessageFormat.format("Hello {0} {1}!", request.getName(), i)).build();
            responseObserver.onNext(response);
